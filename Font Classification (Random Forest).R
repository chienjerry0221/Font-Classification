...
set.seed(20211004)
split_data = data_split(portion = 0.2, nclass = 4, df = final_ZDATA) # Use seed = 20211004 to get same data set as HW2
...
# feature_importance2 = as.data.frame(round(get(paste("rf_", best_tree_number, sep = ""))$importance[c(1:10), "MeanDecreaseAccuracy"] * 100, 1))
# colnames(feature_importance2) = "Importance"
# rownames(feature_importance2) = c("Z1", "Z2", "Z3", "Z4", "Z5", "Z6", "Z7", "Z8", "Z9", "Z10")
# 
# ggplot() +
#   geom_point(aes(x = c(1:10), y = feature_importance2[, "Importance"])) +
#   geom_line(aes(x = c(1:10), y = feature_importance2[, "Importance"])) +
#   geom_text(aes(c(1:10), feature_importance2[,"Importance"], label = feature_importance2[, "Importance"]), hjust = 0.5, vjust = 1.5, size = 8) +
#   theme(text = element_text(size = 25)) +
#   ylab("Importance (%)") + xlab("Feature Z") + expand_limits(x = c(1,10), y = c(0, 2.5)) +
...
## 5.3
# ggplot() +
#   geom_point(aes(x = L[c(1:10)], y = feature_importance2[, "Importance"])) +
#   geom_text(aes(L[c(1:10)] + c(0, 0, 0, 0, 0, 0, 0, 1, -1.4, 0), feature_importance2[, "Importance"] + c(0, 0, 0, 0, 0, 0, 0, 0.8, 0.4, 0), label = rownames(feature_importance2)), hjust = 0.5, vjust = 1.5, size = 5) +
#   theme(text = element_text(size = 25)) +
...
# cor_L_IM = cor(L[c(1:10)], feature_importance2[, "Importance"])
